...
class RunbotRepo(osv.osv):
    ...
    """
    Added pylint_conf_path field to use a configuration of pylint by repository,
...
class RunbotBuild(osv.osv):
    ...
    def get_modules_to_check_pylint(self, cr, uid, ids, context=None):
        ...
        Using field runbot_repo.check_pylint check box and get modules list from
        branch with ls-tree.
        This method use build.modules too to get all depends from selected repo.
        """
        if not ids:
            return True
        if isinstance(ids, (int, long)):
